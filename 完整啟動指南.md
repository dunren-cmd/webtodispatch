# 完整啟動指南 - 確保資料庫資料完全一致

## 🎯 目標

確保從 Git 拉取代碼後，在任何環境都能：
1. 快速啟動應用
2. 資料庫結構完全一致
3. 配置正確（IP、Supabase 位址）

## 📋 完整步驟

### 步驟 1：從 Git 拉取代碼

```bash
git clone <你的倉庫URL>
cd WebToDispatch_2
```

### 步驟 2：安裝依賴

```bash
npm install
```

### 步驟 3：設定環境變數

**Windows:**
```bash
copy .env.example .env
```

**Linux/Mac:**
```bash
cp .env.example .env
```

### 步驟 4：啟動 Supabase

```bash
supabase start
```

**取得配置：**
```bash
supabase status
```

輸出範例：
```
API URL: http://192.168.62.101:54321
anon key: sb_publishable_ACJWlzQHlZjBrEguHvfOxg_3BJgxAaH
```

### 步驟 5：編輯 .env 文件

將步驟 4 取得的配置填入 `.env`：

```env
VITE_SUPABASE_URL=http://192.168.62.101:54321
VITE_SUPABASE_ANON_KEY=sb_publishable_ACJWlzQHlZjBrEguHvfOxg_3BJgxAaH
VITE_APP_PORT=3050
VITE_APP_HOST=0.0.0.0
```

**重要：**
- 如果 Supabase 在不同 IP，修改 `VITE_SUPABASE_URL`
- 每個環境的 `anon key` 可能不同，使用 `supabase status` 取得

### 步驟 6：執行資料庫遷移（關鍵步驟）

**這一步確保資料庫結構完全一致！**

```bash
supabase migration up
```

這會執行 `supabase/migrations/` 目錄中的所有遷移文件：

1. ✅ `20251212134638_init.sql` - 創建 users 和 tasks 表
2. ✅ `20251212170000_merge_personnel_to_users.sql` - 添加 mail, employee_id, headshot
3. ✅ `20251213000000_change_avatar_to_level.sql` - 添加 level，移除 avatar
4. ✅ `20251213120000_update_level5_to_level4.sql` - 更新 level 5 為 4
5. ✅ `20251214000000_fix_users_table_structure.sql` - 修復 users 表結構
6. ✅ `20251215000000_create_roles_table.sql` - 創建 roles 表並建立關聯

**驗證遷移：**
```bash
supabase migration list
```

所有遷移應該標記為已執行。

### 步驟 7：驗證資料庫結構

在 Supabase Studio (`http://localhost:54323`) 的 SQL Editor 中執行：

```sql
-- 檢查所有表
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public'
ORDER BY table_name;

-- 應該看到：users, tasks, roles

-- 檢查外鍵關聯
SELECT
  tc.constraint_name,
  tc.table_name,
  kcu.column_name,
  ccu.table_name AS foreign_table_name
FROM information_schema.table_constraints AS tc
JOIN information_schema.key_column_usage AS kcu
  ON tc.constraint_name = kcu.constraint_name
JOIN information_schema.constraint_column_usage AS ccu
  ON ccu.constraint_name = tc.constraint_name
WHERE tc.constraint_type = 'FOREIGN KEY'
  AND tc.table_schema = 'public';

-- 應該看到：fk_users_role
```

### 步驟 8：啟動應用

```bash
npm run dev
```

應用會在 `http://localhost:3050` 啟動（或你在 `.env` 中設定的端口）

### 步驟 9：驗證應用

1. **檢查瀏覽器控制台**
   - 應該看到環境配置資訊
   - 不應該有 Supabase 連接錯誤

2. **測試功能**
   - 查看任務列表
   - 查看員工列表
   - 查看角色管理（應該能看到 Webhook 設定）

## 🔄 不同環境的配置

### 環境 A（IP: 192.168.68.75）

```env
VITE_SUPABASE_URL=http://192.168.62.101:54321
VITE_SUPABASE_ANON_KEY=sb_publishable_ACJWlzQHlZjBrEguHvfOxg_3BJgxAaH
```

### 環境 B（IP: 192.168.1.100）

```env
VITE_SUPABASE_URL=http://192.168.1.100:54321
VITE_SUPABASE_ANON_KEY=從該環境的 supabase status 取得
```

**重要：**
- 每個環境的 Supabase IP 可能不同
- 每個環境的 `anon key` 可能不同
- 使用 `supabase status` 取得當前環境的配置

## ✅ 檢查清單

拉取代碼後，完成以下檢查：

- [ ] 已執行 `npm install`
- [ ] 已複製 `.env.example` 為 `.env`
- [ ] 已修改 `.env` 中的 Supabase URL（使用當前環境的 IP）
- [ ] 已修改 `.env` 中的 Supabase ANON_KEY（使用 `supabase status` 取得）
- [ ] 已啟動 Supabase：`supabase start`
- [ ] 已執行遷移：`supabase migration up`
- [ ] 已驗證遷移狀態：`supabase migration list`
- [ ] 已驗證資料庫結構（users, tasks, roles 表存在）
- [ ] 已驗證外鍵關聯（fk_users_role 存在）
- [ ] 已啟動應用：`npm run dev`
- [ ] 瀏覽器控制台沒有錯誤
- [ ] 應用功能正常運作

## 🚀 一鍵啟動（使用腳本）

### Windows

執行 `start.bat`，會自動：
1. 檢查並創建 `.env` 文件
2. 檢查 Supabase 狀態
3. 安裝依賴
4. 啟動應用

### Linux/Mac

創建 `start.sh`：

```bash
#!/bin/bash
# 檢查並創建 .env
if [ ! -f ".env" ]; then
    cp .env.example .env
    echo "請編輯 .env 文件後重新執行"
    exit 1
fi

# 安裝依賴
npm install

# 啟動應用
npm run dev
```

## 📝 重要提醒

1. **環境變數必須設定**
   - `.env` 文件不會提交到 Git
   - 每個環境需要自己創建 `.env` 文件
   - 參考 `.env.example` 作為範本

2. **資料庫遷移必須執行**
   - 所有遷移文件都在 `supabase/migrations/` 目錄中
   - 執行 `supabase migration up` 確保資料庫結構一致
   - 遷移文件會提交到 Git，確保所有環境結構相同

3. **Supabase 配置**
   - 每個環境的 IP 和 Key 可能不同
   - 使用 `supabase status` 取得當前環境的配置
   - 修改 `.env` 文件即可切換環境

4. **資料一致性**
   - 遷移文件確保資料庫結構一致
   - 資料內容可能不同（取決於各環境的資料）
   - 結構（表、欄位、關聯）會完全一致

## 🐛 常見問題

### Q1: 如何知道 Supabase 的 IP 和 Key？

**A:** 執行 `supabase status`，會顯示：
```
API URL: http://192.168.62.101:54321
anon key: sb_publishable_ACJWlzQHlZjBrEguHvfOxg_3BJgxAaH
```

### Q2: 為什麼要執行遷移？

**A:** 遷移文件定義了資料庫結構。執行遷移確保：
- 所有表都存在
- 所有欄位都正確
- 所有關聯都建立
- 資料庫結構完全一致

### Q3: 如果 Supabase 在不同 IP 怎麼辦？

**A:** 修改 `.env` 文件中的 `VITE_SUPABASE_URL`：
```env
VITE_SUPABASE_URL=http://新的IP:54321
```
然後重新啟動應用。

### Q4: 如何確保資料庫資料完全一樣？

**A:** 
1. 執行所有遷移文件：`supabase migration up`
2. 遷移文件會創建相同的表結構
3. 資料內容可能不同（取決於各環境）
4. 結構會完全一致

### Q5: .env 文件會被提交到 Git 嗎？

**A:** 不會。`.env` 已在 `.gitignore` 中排除。`.env.example` 會提交，作為範本。
