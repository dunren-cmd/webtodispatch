...
你輸入 `http://192.168.68.75:3050/` 看到的是**前端應用**，不是 Supabase API。
...
2. **或者使用本機 IP 訪問**
   ...
   - 如果綁定到 `0.0.0.0`，可以使用 `http://192.168.68.75:54321`
...
```
http://192.168.68.75:54321/rest/v1/users
```
...
# 測試連接
Invoke-RestMethod -Uri "http://192.168.68.75:54321/rest/v1/users" `
  -Headers @{
    "apikey" = $anonKey
...
1. **`Code.gs`**：`SUPABASE_URL = "http://192.168.68.75:54321"`
2. **`api.ts`**：`SUPABASE_URL = 'http://192.168.68.75:54321'`
...
3. **網路訪問問題**
   - 如果 `192.168.68.75:54321` 無法訪問，嘗試使用 `localhost:54321`
   - 確認防火牆允許端口 54321
